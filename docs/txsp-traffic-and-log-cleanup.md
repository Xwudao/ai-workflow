...
## 七、2026-09-25 复核：套餐耗尽，开始按小时计费

**现象**：腾讯云账单出现「公网 IP-新加坡-09月按小时结算扣费-20260925…」，每小时 **0.74~0.88 元**
（≈0.8 元/GB 的超额流量单价），即从 09-25 下午起 1 TB 流量包已耗尽，开始计费。

**实测（21:30 CST，即 13:30 UTC）**：

| 指标 | 值 |
|---|---|
| ens5 60s 取样 | RX 18.8 MiB / TX 18.1 MiB ≈ **1.0~1.1 GB/h·方向 ≈ 25 GiB/天** |
| 55s tcpdump（31 MB） | 最大对端 `gate 110.40.167.56`：in 8.5 + out 5.2 MB，占 **44%**；其余为 Cloudflare 各边缘 IP 回源 |
| 出方向构成 | 16.3 MB 中 **5.2 MB（≈1/3）是发往 gate 的请求**（含头/cookie，约 0.8 KB/请求），其余为发回 CF 的响应体 |
| 55s 请求量 | 6,616 请求（≈108 req/s，为晚高峰样本；当日 13.4h 全量约 129 万请求） |

**站点与路径（当日日志口径，受 roll_keep 限制为下限）**：

| 站点 | 字节 | 主要路径 |
|---|---:|---|
| lzpanx | 6.25 GB | `/search` 0.57、`/v1/captcha/gen` 0.57、`/assets/reman_wasm_bg-*.wasm` 0.43、`/v1/site_config/all` 0.29、`/favicon.ico` 0.17 GB |
| qkpanso | 0.24 GB | `/search` 0.07 |
| hunhepan / fuxipan | 0.08 / 0.06 GB | — |

- 单 IP 最大 **112.98.84.160（哈尔滨电信）1.49 GB**，当日仍返回 200，**未**在黑名单内
  （同段 112.98.87.93 / 82.112 / 86.126 / 82.255 已在两处黑名单）。
- **fuxipan 因后端 502（见第六节）当日仅 0.06 GB**，流量重心已从 fuxipan 转到 lzpanx。
- 状态码：200 = 512k、403 = 496k、429 = 259k：爬虫与限流占比高；被拦请求在 lzpanx 上仍会走一轮 txsp→gate。

**结论与建议（本次未执行任何变更）**：

1. 把 `112.98.84.160` 加入 `txsp` + `gate` 两处 WAF 黑名单（热加载，无需 reload）。
2. 静态资源（`/assets/*.wasm`、`/favicon.ico`）在 Cloudflare 侧加 Cache Rule，省掉全部回源。
3. 降低请求数（CF Rate Limiting / Bot Fight），因为**每个请求本身**就要付 ≈0.8 KB 到 gate + 一轮头部的出流量。
4. 结构性优化：txsp→gate 目前走公网（`110.40.167.56` 计费），若两机同属腾讯云可考虑内网/对等连接，
   可直接省掉出方向约 1/3 的请求流量。
5. 成本：按当前 25 GB/天 × 0.8 元/GB ≈ **20 元/天 ≈ 600 元/月**；建议在控制台确认流量包重置周期。
